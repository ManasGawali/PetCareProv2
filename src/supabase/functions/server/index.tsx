...
// Initialize sample data - now checks database first
const initializeSampleData = async () => {
  try {
    ...
    console.log(`Database initialized with ${servicesData?.length || 0} services and ${productsData?.length || 0} products`);
  } catch (error) {
...
app.get("/make-server-0c3770d9/health", (c) => {
  return c.json({ status: "ok", timestamp: new Date().toISOString() });
});

// Authentication endpoints
app.post("/make-server-0c3770d9/auth/signup", async (c) => {
  try {
...
app.post("/make-server-0c3770d9/auth/signin", async (c) => {
  try {
...
app.get("/make-server-0c3770d9/auth/user", async (c) => {
  try {
...
// Services endpoints
app.get("/make-server-0c3770d9/services", async (c) => {
  try {
...
app.get("/make-server-0c3770d9/services/:id", async (c) => {
  try {
...
// Products endpoints
app.get("/make-server-0c3770d9/products", async (c) => {
  try {
...
app.get("/make-server-0c3770d9/products/:id", async (c) => {
  try {
    const id = c.req.param('id');
...
// Bookings endpoints
app.post("/make-server-0c3770d9/bookings", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.get("/make-server-0c3770d9/bookings/user", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.get("/make-server-0c3770d9/bookings/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.put("/make-server-0c3770d9/bookings/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    const bookingId = c.req.param('id');
...
// Cart endpoints
app.get("/make-server-0c3770d9/cart", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.post("/make-server-0c3770d9/cart/items", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.put("/make-server-0c3770d9/cart/items/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.delete("/make-server-0c3770d9/cart/items/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    const itemId = c.req.param('id');
...
app.delete("/make-server-0c3770d9/cart/clear", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
// Orders endpoints
app.post("/make-server-0c3770d9/orders", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.get("/make-server-0c3770d9/orders/user", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
// Pets endpoints
app.get("/make-server-0c3770d9/pets", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.post("/make-server-0c3770d9/pets", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.put("/make-server-0c3770d9/pets/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    ...
    if (error) {
      ...
      return c.json({ success: false, message: 'Failed to update pet profile' }, 500);
    }
...
app.delete("/make-server-0c3770d9/pets/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    const petId = c.req.param('id');
    ...
    if (error) {
      ...
      return c.json({ success: false, message: 'Failed to delete pet profile' }, 500);
    }
...
// Tracking endpoints
app.get("/make-server-0c3770d9/tracking/:bookingId", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    ...
    if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
      ...
      return c.json({ success: false, message: 'Failed to fetch tracking' }, 500);
    }
    
    return c.json({ success: true, data: tracking || null });
  } catch (error) {
    console.error('Get tracking error:', error);
    return c.json({ success: false, message: error.message || 'Internal server error' }, 500);
  }
...
// Reviews endpoints
app.post("/make-server-0c3770d9/reviews", async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
...
app.get("/make-server-0c3770d9/reviews/service/:serviceId", async (c) => {
  try {
